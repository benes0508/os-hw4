...
void test_fifo_order() {
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        thrd_sleep(&(const struct timespec){.tv_nsec = 0.005 * SECOND_IN_NANOSECONDS}, NULL);
    }
    ...
    for (int i = 0; i < NUM_THREADS; i++) {
        ...
        printf("Consumer thread %d finished, dequeued item %d\n", i, dequeue_order[i]);
    }

    // Verify FIFO order
    for (int i = 0; i < NUM_THREADS; i++) {
        printf("Verifying thread %d dequeued item %d\n", i, dequeue_order[i]);
...
int consumer_thread(void *arg) {
    ...
    printf("Consumer thread started, waiting to dequeue...\n");
    void *item = dequeue();

    *dequeue_order = *(int *)item;
    printf("Consumer thread dequeued item %d\n", *dequeue_order);
    free(item);
...
int producer_thread(void *arg) {
    ...
    printf("Producer thread started, enqueueing items...\n");
    // Enqueue at least NUM_THREADS items
    for (int i = 0; i < NUM_THREADS; i++) {
        int *item = malloc(sizeof(int));
        *item = i + 1;
        printf("Producer thread enqueueing item %d\n", *item);
        enqueue(item);
    }

    printf("Producer thread finished enqueueing items\n");
    return 0;
}
